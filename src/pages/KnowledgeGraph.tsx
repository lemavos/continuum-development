...
const TYPE_COLORS: Record<string, string> = {
  NOTE: "hsl(210, 100%, 52%)",
  ENTITY: "hsl(152, 69%, 45%)",
  HABIT: "hsl(38, 92%, 50%)",
  PERSON: "hsl(210, 100%, 52%)",
  PROJECT: "hsl(38, 92%, 50%)",
  TOPIC: "hsl(0, 0%, 64%)",
  ORGANIZATION: "hsl(152, 69%, 45%)",
};
...
export default function KnowledgeGraph() {
  ...
  const findNodeAt = useCallback((sx: number, sy: number) => {
    ...
    for (let i = nodesRef.current.length - 1; i >= 0; i--) {
      ...
      const r = (TYPE_RADIUS[n.type] || 6) / z + 4;
      const dx = n.x - w.x;
      const dy = n.y - w.y;
  ...
  const draw = useCallback(() => {
    ...
    for (const n of nodes) {
      ...
      const color = TYPE_COLORS[n.type] || "hsl(0,0%,64%)";
      const isHovered = hoveredNode?.id === n.id;
      const isSelected = selectedNode?.id === n.id;

      ctx.beginPath();
      ctx.arc(n.x, n.y, isHovered || isSelected ? r + 2 : r, 0, Math.PI * 2);
      ctx.fillStyle = color;
      ctx.fill();
      ...
      if (zoomRef.current > 0.5 || isHovered || isSelected) {
        ...
        ctx.font = `${11 / zoomRef.current}px Inter, sans-serif`;
        ctx.textAlign = "center";
        ctx.fillText(n.label.slice(0, 20), n.x, n.y + r + 14 / zoomRef.current);
      }
    }
  ...
  useEffect(() => {
    graphApi.data().then(({ data }: { data: GraphData }) => {
      ...
      toast({ title: "Erro ao carregar grafo", variant: "destructive" });
      setLoading(false);
    });
  }, []);
  ...
  const handleWheel = (e: React.WheelEvent) => {
    ...
    const newZ = Math.max(0.2, Math.min(4, oldZ * (e.deltaY < 0 ? 1.1 : 0.9)));
  ...
  const handleZoom = (dir: number) => {
    zoomRef.current = Math.max(0.2, Math.min(4, zoomRef.current * (dir > 0 ? 1.3 : 0.7)));
  };
  ...
  return (
    <AppLayout>
      <div className="flex flex-col h-full">
        <div className="p-4 lg:p-6 flex items-center justify-between border-b border-border shrink-0">
          <div>
            <h1 className="text-xl lg:text-2xl font-semibold tracking-tight text-foreground">Grafo de Conhecimento</h1>
            <p className="text-xs text-muted-foreground">
              {nodesRef.current.length} nós · {edgesRef.current.length} conexões
            </p>
          </div>
